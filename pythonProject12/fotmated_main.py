...
def select_knowledge_base_attributes(knowledge_base):
    attribute_attr_list = [
        a.attrib for a in knowledge_base.find("attributes").iter("name")
    ]
...
def populate_matrix_with_conditions(knowledge_base, data_frame):
    for i in range(len(list(data_frame.ruleID))):
        rule_x = get_rule_by_id(knowledge_base, data_frame.ruleID[i])
        cond = rule_x.find("conditions")
        for k in list(cond):
            data_frame.loc[
                data_frame["ruleID"] == data_frame.ruleID[i],
                str("attr_" + list(k)[0].attrib["attributeID"]),
            ] = list(k)[2].text
    return data_frame
...
def compute_result_k(data, distance_type, linkage_type, k):
    ...
    k_l = cluster.hierarchy.fcluster(grouped, k, "maxclust", R=None, monocrit=None)
    plt.title(
        f"Hierarchical Clustering Dendrogram - {distance_type} distance, {linkage_type} linkage"
    )
    print(f'time:\t{round(elapsed_time, 6)}')
    plt.show()
    return elapsed_time, d, k_l, dist
    ...
    while True:
        ...
        if linkage_ans == "1":
            ...
            for i in range(len(d_table.index.values)):
                k1 = round(
                    math.sqrt(len(d_table.index.values))
                    + ((i + 1) * r_n(len(d_table.index.values)))
                )
                k2 = round(
                    math.sqrt(len(d_table.index.values))
                    - ((i + 1) * r_n(len(d_table.index.values)))
                ...
                if k2 <= 2 or k1 >= len(d_table.index.values):
                    break
                t1, tree1, k_labels1, distance1 = compute_result_k(
                    d_table, distance_type, "single", k1
                )
                t2, tree2, k_labels2, distance2 = compute_result_k(
                    d_table, distance_type, "single", k2
                )
                print(f"k1: {k1}: {k_labels1}")
                print(f"k2: {k2}: {k_labels2}")
                try:
                    silhouette_avg_1 = silhouette_score(
                        squareform(distance1), k_labels1, metric="precomputed"
                    )
                    silhouette_avg_2 = silhouette_score(
                        squareform(distance2), k_labels2, metric="precomputed"
                    )
                    ...
                    result_k = result_k.append(
                        {
                            ...
                            "Cluster_count": k1,
                            "Distance_messure": distance_type,
                            "Linkage_method": "single",
                            "Largest_group_count": Counter(k_labels1).get(
                                max(Counter(k_labels1), key=Counter(k_labels1).get)
                            ),
                            "Smalest_group_count": Counter(k_labels1).get(
                                min(Counter(k_labels1), key=Counter(k_labels1).get)
                            ),
                            "Computation_time": t1,
                            "silhouette_score": silhouette_avg_1,
                        },
                        ignore_index=True,
                    )
                    result_k = result_k.append(
                        {
                            ...
                            "Distance_messure": distance_type,
                            "Linkage_method": "single",
                            "Largest_group_count": Counter(k_labels2).get(
                                max(Counter(k_labels2), key=Counter(k_labels2).get)
                            ),
                            "Smalest_group_count": Counter(k_labels2).get(
            ...
            for i in range(len(d_table.index.values)):
                ...
                if k2 <= 2 or k1 >= len(d_table.index.values):
                    break
                t1, tree1, k_labels1, distance1 = compute_result_k(
                    d_table, distance_type, "complete", k1
                )
                t2, tree2, k_labels2, distance2 = compute_result_k(
                    d_table, distance_type, "complete", k2
                )
                print(f"k1: {k1}: {k_labels1}")
                print(f"k2: {k2}: {k_labels2}")
                try:
            ...
            for i in range(len(d_table.index.values)):
                ...
                if k2 <= 2 or k1 >= len(d_table.index.values):
                    break
                t1, tree1, k_labels1, distance1 = compute_result_k(
                    d_table, distance_type, "average", k1
                )
                t2, tree2, k_labels2, distance2 = compute_result_k(
                    d_table, distance_type, "average", k2
                )
                print(f"k1: {k1}: {k_labels1}")
                print(f"k2: {k2}: {k_labels2}")
                try:
            ...
            for i in range(len(d_table.index.values)):
                ...
                if k2 <= 2 or k1 >= len(d_table.index.values):
                    break
                t1, tree1, k_labels1, distance1 = compute_result_k(
                    d_table, distance_type, "ward", k1
                )
                t2, tree2, k_labels2, distance2 = compute_result_k(
                    d_table, distance_type, "ward", k2
                )
                print(f"k1: {k1}: {k_labels1}")
                print(f"k2: {k2}: {k_labels2}")
                try:
...
result_k.to_csv(f"new_tests/{kb_name}.csv")
